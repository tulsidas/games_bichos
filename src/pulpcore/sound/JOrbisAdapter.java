...
public class JOrbisAdapter extends Sound {
    ...
    private static boolean isSupportedFormat(String soundAsset, int sampleRate,
        ...
        int[] sampleRates = CoreSystem.getPlatform().getSoundEngine()
        ...
        if (Build.DEBUG) {
            CoreSystem.print("Unsupported sample rate (" + sampleRate + "Hz): "
                    + soundAsset);
        }
        return false;
    }
    ...
    public Sound decompress() {
        ...
        return Sound.load(dest, file.getSampleRate(),
                (file.getNumChannels() == 2));
    }
    ...
    public Playback play(Fixed level, Fixed pan, boolean loop) {
        if (!file.isRunning()) {
            ...
            return new JOrbisAdapter(filename, file.duplicate()).playImpl(
                    level, pan, loop);
        }
    }
